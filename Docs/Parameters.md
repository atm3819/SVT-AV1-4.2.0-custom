...
| **KeyFrameMinInterval**          | --keyint-min          | [0-`(2^31)-1`]  | 0                 | Minimum frames between key frames (libaom kf_min_dist-style floor); paces scene-change and forced key frames, periodic `--keyint` is never gated [0: off]      |
| **ForcedKeyFramePolicy**         | --forced-kf-policy    | [0-2]           | 0                 | Handling of a forced key frame requested within `--keyint-min` of the previous key frame [0: honor, 1: coalesce/drop, 2: defer]                              |
